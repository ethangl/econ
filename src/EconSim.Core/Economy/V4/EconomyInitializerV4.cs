...
namespace EconSim.Core.Economy.V4
{
    ...
    /// </summary>
    public static class EconomyInitializerV4
    {
        public static EconomyStateV4 Initialize(SimulationState state, MapData mapData)
        {
            int maxCountyId = 0;
            foreach (var county in mapData.Counties)
                if (county.Id > maxCountyId) maxCountyId = county.Id;

            var econ = new EconomyStateV4();
            econ.Counties = new CountyEconomyV4[maxCountyId + 1];

            foreach (var county in mapData.Counties)
            {
                var ce = new CountyEconomyV4();
            ...
            SimLog.Log("EconomyV4",
                $"Initialized: {mapData.Counties.Count} counties, {econ.MarketCount} markets, " +
                $"{GoodsV4.Count} goods, {FacilitiesV4.Count} facilities");
        ...
        static void ComputeProductivity(County county, MapData mapData, float[] output)
        {
            ...
            foreach (int cellId in county.CellIds)
            {
                ...
                for (int g = 0; g < GoodsV4.Count; g++)
                {
                    float y = GoodsV4.BiomeYield[cell.BiomeId, g];
                    output[g] += y;
                }
            }
        ...
        static void InitializeMarkets(MapData mapData, EconomyStateV4 econ, int maxCountyId, SimulationState state)
        {
            int maxRealmId = 0;
            ...
            econ.Markets = new MarketStateV4[marketCount + 1];
            ...
            for (int m = 0; m < mapData.Realms.Count; m++)
            {
                ...
                var market = new MarketStateV4(marketId);
